...
export function ResizableTableHeader({
  ...
  return <TableHead style={{
    width: `${width}px`,
    position: 'relative'
  }} className={`${className} ${isDragging ? 'opacity-50' : ''}`} draggable={draggable} onDragStart={handleDragStart} onDragOver={onDragOver} onDrop={onDrop} onDragEnd={handleDragEnd}>
      <div className="flex items-center justify-between gap-1 pr-3">
        {draggable && <GripVertical className="h-4 w-4 text-muted-foreground cursor-move flex-shrink-0" />}
